<template>
  <div>
    ...
    <!-- za sve oglase ponude -->
    <kartica-ponuda v-for="card in cards2" :key="card.id" :info="card" />
    <!-- za filtirane oglase ponude -->

    <!-- v-for direktiva jer želimo ponavljati kartice ponude 
    card in cards - ponavljaj karticu ponude za svaki card u cards
...
export default {
  ...
  methods: {
    // metoda za dohvaćanje svih oglasa odnosno dokumenta sa firestore-a iz kolekcije oglasiPonuda
    async getAds() {
    ...
    async getFilteredAds() {
      ...
      querySnapshot.forEach((doc) => {
        const data = doc.data();
        if (data.workPlace.toLowerCase().includes(term))
